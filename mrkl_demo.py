...
from sqlalchemy import create_engine
import sqlite3
import requests  # Import requests module for sending HTTP requests
...
# Setup credentials in Streamlit
openai_api_key = "not_supplied"  # Placeholder for the OpenAI API key
...
with st.form(key="form"):
    "Ask one of the sample questions, or enter your own custom question."

    prefilled = st.selectbox("Sample questions", sorted(SAVED_SESSIONS.keys())) or ""
    user_input = st.text_input("Or, ask your own question", value=prefilled)
    submit_clicked = st.form_submit_button("Submit Question")
...
if with_clear_container(submit_clicked):
    ...
    if user_input in SAVED_SESSIONS:
        ...
        response = requests.post(api_gateway_url, json={"question": user_input}).json()
        answer = response.get("output") if "output" in response else response
